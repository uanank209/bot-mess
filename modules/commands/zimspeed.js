...
module.exports.onLoad = async () => {
    const dir = __dirname + `/xedua/datauser/`;
    const _dir = __dirname + `/xedua/datauser/`;
    const __dir = __dirname + `/xedua/cache/`;
...
module.exports.checkPath = function (type, senderID) {
    const pathGame = path.join(__dirname, 'xedua', 'datauser', `${senderID}.json`);
    const pathGame_1 = require("./xedua/datauser/" + senderID + '.json');
...
module.exports.image = async function(link) {
    ...
    let download = (await axios.get(link, { responseType: "arraybuffer" } )).data; 
        writeFileSync( __dirname + `/xedua/cache/xedua.png`, Buffer.from(download, "utf-8"));
        images.push(createReadStream(__dirname + `/xedua/cache/xedua.png`));
    return images
}
...
module.exports.run = async function ({ api, event, args, client,Threads,__GLOBAL, Users, Currencies,getText }) {
   const { senderID, messageID, threadID } = event;
    ...
    const pathData = path.join(__dirname, 'xedua', 'datauser', `${senderID}.json`);
    switch (args[0]) {
        case 'register':
        case '-r': {
        ...
        case 'spin': {
            ...
            const data = readdirSync(__dirname + `/xedua/datauser`);
            if(i == 3) {
                ...
                for (let i of data) { 
                    if(i != `${senderID}.json`) {
                        dem.push(require(`./xedua/datauser/${i}`));
                    }
                }
                ...
                for (let i of data) { 
                    if(i != `${senderID}.json`) {
                        var o = require(`./xedua/datauser/${i}`);
                        p.push(o)
        ...
        case 'xaydung': {
            ...
            var a = require(`./xedua/datauser/${senderID}.json`);
     ...
     return api.sendMessage({body: `── [ CHỌN MAPS THI ĐẤU ] ──  \n\n❤️ Chọn maps để thi đấu\n[🗺1]. Rừng Tàn Phá\n[🗺2].Lông Môn Tân Xuân\n[🗺3].Ngôi Nhà Linh Thiên\n`, attachment: await this.image('https://i.imgur.com/9YaNyZA.jpg')}, threadID, (error, info) => {
        ...
        case 'info': {
            if (!existsSync(pathData)) {
                return api.sendMessage({body: "Bạn chưa đăng kí game!", attachment: await this.image('https://taigamemienphi.org/wp-content/uploads/2022/05/zing-code-1.jpg')}, threadID, messageID);
            }
            var a = require(`./xedua/datauser/${senderID}.json`);
        ...
        case 'top': {
            if (!existsSync(pathData)) {
                return api.sendMessage({body: "Bạn chưa đăng kí game!", attachment: await this.image('https://www.bing.com/th/id/OGC.878ccba22484051403163d04d0a824f4?pid=1.7&rurl=https%3a%2f%2fmedia.giphy.com%2fmedia%2f3kE6uH1ZkmcdXIMQLL%2fgiphy.gif&ehk=g4mXzxH2ET4RVCUZ9AUDs6l5gwT4lSzlTnuvIisqtMs%3d')}, threadID, messageID);
            }
            const data = readdirSync(__dirname + `/xedua/datauser`);
        ...
        default: {
            return api.sendMessage({body: `===[ ĐUA XE S1 ]===\n» R: Đăng kí\n» SPIN: Vòng quay game\n» BUILD: Nâng cấp động cơ\n» SHOP: Shop mua súng\n» INFO: Xem thông tin về bạn\n» TOP: Xem top level trên server\n» CHANGE: Quy đổi tiền của bot sang tiền game và ngược lại`, attachment: await this.image('https://i.imgur.com/28F3cY4.jpg')}, threadID, messageID);
        }
    }
}
...
Sức mạnh boost: 2.9
Drift: 32%
Sức mạnh tông: 15%`, attachment: await this.image('https://i.imgur.com/t7wrAX1.jpg')}, threadID, messageID);
            }
            else if(body == 3) {

                return api.sendMessage({body: `⭐️ MUA THÀNH CÔNG ⭐️\n[🚘]Tên : ETARNA
[🍁]Thông Tin : Lửa h��y diệt 
Tốc độ: 2.1/h tối đa
Sức mạnh boost: 3.1
Drift: 31%
...
Sức mạnh tông: 100%`, attachment: await this.image('https://i.imgur.com/HfK8lJD.jpg')}, threadID, messageID);
         ...
         case 'mua': {
            if(body == 1) {
                ...
                return api.sendMessage('Vui lòng reply tin nhắn n��y với số lượt quay bạn muốn mua! (10 lượt quay 2000$)', threadID, (error, info) => {
                    global.client.handleReply.push({
                        name: this.config.name,
                        messageID: info.messageID,
                        author: event.senderID,
